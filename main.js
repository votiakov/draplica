...
if (window.addEventListener) {
	window.addEventListener('load', function () {
		...
		var backgroundForDwnld;

		document.getElementById("background").addEventListener("change", function () {
			backgroundForDwnld = document.getElementsByTagName('CANVAS')[1].style.backgroundColor;

			console.log(backgroundForDwnld);

		});

		document.getElementById("transparency").addEventListener("click", function () {
			console.log(bckgrFlag);
			bckgrFlag = !bckgrFlag;
			console.log(bckgrFlag);
		...
		function download(canvas, filename) {
			...
			if (bgImage) 	 //////////////////////////////////////////////////////////////////// SHOULD WORK ON THIS! CANNOT DOWNLOAD CANVAS WITH IMAGE UPLOADED THERE
			{

				// draw image
				var img = new Image();
				img.setAttribute('crossOrigin', 'anonymous');
				img.src = 'notebook_texture.jpg';
				//ctxDummy.drawImage(img,dummy.width,dummy.height);
				img.onload = function () {
					console.log("OK");
					ctxDummy.drawImage(this, 0, 0, dummy.width, dummy.height);
					console.log(dummy.toDataURL());
				}

			}
			else {
				ctxDummy.fillStyle = backgroundForDwnld;
				ctxDummy.fillRect(0, 0, dummy.width, dummy.height);

				ctxDummy.drawImage(canvas, 0, 0);
			}
			...
			if (bckgrFlag == false) { lnk.href = canvas.toDataURL(); }
			else { lnk.href = dummy.toDataURL(); }
		...
		var onPaint = function () {
			...
			else if (tool == 'star') { drawStar(star_count, tmp_ctx.lineWidth / 50); }
